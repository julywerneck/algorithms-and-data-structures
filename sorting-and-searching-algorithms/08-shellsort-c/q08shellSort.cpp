...
void insertionByColor(Player *vet[], int n, int cor, int h)
{
    for (int i = (h + cor); i < n; i += h)
    {
        Player *tmp = vet[i];
        int j = i - h;
        while ((j >= 0) && (vet[j]->peso > tmp->peso || (vet[j]->peso == tmp->peso && (strcmp(vet[j]->nome, tmp->nome) > 0))))
        {
            vet[j + h] = vet[j];
            j -= h;
        }
...
void shellSort(Player *vet[], int n)
{
    int h = 1;
...
int main()
{
    ...
    shellSort(player, numEntrada);

    fim = clock();

    double tempo = ((fim - inicio) / (double)CLOCKS_PER_SEC);
    pont_arq_log = fopen("683595_shellsort.txt", "w");
