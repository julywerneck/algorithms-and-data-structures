...
public class q03pesquisasequencial { 
	...
	String[] pesquisa = new String[1000];
       	int numPesq = 0;  
	Players[] arrayPlayer = new Players[300];
	Listas lista = new Listas(300);	//Cria lista de tam 300  	
	...
	/* Lê todos os valores de entrada até que encontre 'FIM' */
	    ...
	    lista.inserirFim(arrayPlayer[i]); 
	}

	do {
		pesquisa[numPesq] = MyIO.readLine();
	} while(isFim(pesquisa[numPesq++]) == false);

	numPesq--;  
	long inicio = now(); 
	for(int j = 0; j < numPesq; j++) {
		if(lista.pesqSeq(pesquisa[j]) == true) {
			MyIO.println("SIM"); 
		} else {
			MyIO.println("NAO");
		}
	...
	Arq.openWrite("683595_selecao.txt", "UTF-8");
	Arq.print("Matricula : 683585\t");
	Arq.print("Tempo de execução : "+tempo+"s \t");
	Arq.print("Numero de comparações : "+lista.getComparacoes());
	Arq.close(); 	
    }
}  
...
class Listas {
	...
	// Insere personagens ao fim da lista
	public void inserirFim(Players plr) throws Exception {
		if(n >= array.length) {
			throw new Exception ("Erro ao inserir");
		} 

		this.array[this.n] = plr; 
		this.n++; 
	} 

	public boolean pesqSeq(String str) {
		boolean find = false; 

		for(int i = 0; i < this.n && find == false; i++) {
			comparacoes++; 
			if(array[i].getNome().equals(str)) {
				find = true; 
			} 
		} 
