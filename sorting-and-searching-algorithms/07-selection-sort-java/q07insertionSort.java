...
class q07insertionSort {
	...
	public static void main(String[] args) throws Exception {
		...
		long inicio = now();
		insertionSort(arrayPlayer, numEntrada, comparacoes);
		long fim = now();

		for (int j = 0; j < numEntrada; j++) {
			arrayPlayer[j].imprimir();
		}
	...
	/*
	 * Algoritmo do insertion sort - critério de comparação o nome do jogador e para
	 * desempate o ano de nascimento
	 */
	public static void insertionSort(Players[] player, int tam, int comparacoe) {
		for (int i = 1; i < tam; i++) {

			Players tmp = player[i];
			int j = i - 1;

			while ((j >= 0) && (player[j].getAnoNascimento() > tmp.getAnoNascimento()
					|| (player[j].getAnoNascimento() == tmp.getAnoNascimento()
							&& (player[j].getNome().compareTo(tmp.getNome()) > 0)))) {
				player[j + 1] = player[j];
				j--;
			}
			player[j + 1] = tmp;
