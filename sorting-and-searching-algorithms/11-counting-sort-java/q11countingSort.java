...
class q11countingSort {
    ...
    public static void main(String[] args) throws Exception {
        ...
        long inicio = now();
        countingSort(arrayPlayer, numEntrada, comparacoes);
        long fim = now();
    ...
    /*
     * Algoritmo countingSort - critério de comparação o nome do jogador e para
     * desempate o ano de nascimento
     */

    public static void countingSort(Players[] p, int n, int comp) {
        int[] count = new int[getMaior(p, n) + 1];
        Players[] sort = new Players[n];
        ...
        for (int i = n - 1; i >= 0; i--) {
            sort[count[p[i].getAltura()] - 1] = p[i];
            count[p[i].getAltura()]--;
        }
        ...
        for (int i = 0; i < (n - 1); i++) {
            int menor = i;
            for (int j = (i + 1); j < n; j++) {
                if (sort[menor].getAltura() == sort[j].getAltura()
                        && sort[menor].getNome().compareTo(sort[j].getNome()) > 0) {
                    menor = j;
                }
            }
            swap(menor, i, sort);
    ...
    public static int getMaior(Players[] p, int n) {
        int maior = (int) p[0].getAltura();
