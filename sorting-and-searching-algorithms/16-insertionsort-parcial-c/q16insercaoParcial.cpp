...
void insertionSort(Player *p[], int n)
{
    for (int i = 1; i < n; i++)
    {
        Player *tmp = p[i];
        ...
        if (i > 10)
        {
            ...
            j = i - 1;
        int k = -1;
        while ((j >= 0) && (p[j]->anoNascimento > tmp->anoNascimento || (p[j]->anoNascimento == tmp->anoNascimento && strcmp(p[j]->nome, tmp->nome) > 0)))
        {
            p[j + 1] = p[j];
            k++;
            j--;
...
int main()
{
    ...
    insertionSort(player, numEntrada); //chama a função de ordenação recursiva
    inicio = clock();
    fim = clock();
