...
//Algoritmo de seleção recursivo
void selectionSort(Player *p[], int n, int i)
{
    if (i < n - 1) //condição de parada da função recursiva
    {
        int menor = i;
        for (int k = (i + 1); k < n; k++)
        {
            comparacoes++;
            if (strcmp(p[menor]->nome, p[k]->nome) > 0)
            {
                menor = k;
            }
        }
        /*faz o swap dos jogadores*/
        Player *tmp = p[menor];
        p[menor] = p[i];
        p[i] = tmp;
        selectionSort(p, n, i + 1); //chama a função recursiva incrementando o i
    }
    else
    {
        for (int u = 0; u < n; u++)
...
int main()
{
    ...
    inicio = clock();
    selectionSort(player, numEntrada, 0); //chama a função de ordenação recursiva
    fim = clock();

    double tempo = ((fim - inicio) / (double)CLOCKS_PER_SEC);
    pont_arq_log = fopen("683595_selecaoRecursiva.txt", "w");
