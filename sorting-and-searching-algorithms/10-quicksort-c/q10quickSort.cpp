...
void quickSortRec(int esq, int dir)
{
    ...
    Player *pivo = player[(dir + esq) / 2];

    while (i <= j)
    {
        while (strcmp(player[i]->estadoNascimento, pivo->estadoNascimento) < 0 || (strcmp(player[i]->estadoNascimento, pivo->estadoNascimento) == 0 && (strcmp(player[i]->nome, pivo->nome) < 0)))
        {
            i++;
        }

        while (strcmp(player[j]->estadoNascimento, pivo->estadoNascimento) > 0 || (strcmp(player[j]->estadoNascimento, pivo->estadoNascimento) == 0 && (strcmp(player[j]->nome, pivo->nome) > 0)))
        {
            j--;
        }
