...
class q18quickSortParcial {
    ...
    public static void main(String[] args) throws Exception {
        ...
        long inicio = now();
        quickSort(arrayPlayer, numEntrada);
        long fim = now();
    ...
    public static void quickSort(Players[] p, int n) {
        quickSortRec(p, 0, n - 1);
    }

    public static void quickSortRec(Players[] p, int esq, int dir) {
        int i = esq;
        int j = dir;
        Players pivo = p[(esq + dir) / 2];
        ...
        if (esq < j) {
            quickSortRec(p, esq, j);
        }

        if (i < 10 && i < dir) {
            quickSortRec(p, i, dir);
        }
