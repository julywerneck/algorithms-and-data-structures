...
public class q15selecaoParcial {
    ...
    public static void main(String[] args) throws Exception {
        ...
        long inicio = now();
        selectionSort(arrayPlayer, numEntrada, comparacoes);
        long fim = now();
        ...
        Arq.openWrite("683595_selecaoParcial.txt", "UTF-8");
    ...
    /*
     * Algoritmo do selection sort - procura sempre o menor elemento para ser
     * inserido no array, sendo comparado por nomes do jogador
     */
    public static void selectionSort(Players[] player, int tam, int comparacoes) {
        for (int i = 0; i < 10; i++) {
            int menor = i;
            for (int j = (i + 1); j < tam; j++) {
                comparacoes++;
                if (player[menor].getNome().compareTo(player[j].getNome()) > 0) {
                    menor = j;
                }
            }
            swap(menor, i, player);
